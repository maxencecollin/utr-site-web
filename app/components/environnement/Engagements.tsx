...
export default function Engagements() {
  ...
  return (
    <section id="engagements" className="relative isolate overflow-hidden py-20 text-white lg:py-28">
      <Image src="/photos/env-engagements.jpg" alt={t("engAlt")} fill sizes="100vw" className="-z-20 object-cover object-[50%_35%]" />
      ...
      <div className="mx-auto max-w-6xl px-6 lg:px-10">
        ...
        <ul className="mt-14 max-w-xl space-y-8">
          {ENGAGEMENTS.map((e) => (
            <li key={e.titre} className="flex items-start gap-5">
              ...
              <span className="flex h-14 w-14 shrink-0 items-center justify-center">
                {e.picto && (
                  <Image src={e.picto} alt="" width={56} height={56} className="h-14 w-auto brightness-0 invert" />
                )}
              </span>
              <div className="flex-1">
                <h3 className="titre text-[18px] sm:text-[21px]">{t(e.titre)}</h3>
                <span className="mt-2 block h-px bg-white/60" />
                <p className="mt-2.5 text-[15px] leading-[1.5] text-white/90">{t(e.texte)}</p>
              </div>
            </li>
          ))}
        </ul>
